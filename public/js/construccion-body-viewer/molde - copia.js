...
//* Convierto las medidas en medios contornos (en lso sitemas de patronaje se trabajan medios contornos)*//
for (let clave in medidasHz) {
	medidasHz[clave] = medidasHz[clave] / 2;
}

let hTotal = 1; //holguraTotal
let holgura = 1;
const hPecho = 3; // valor arbitrario constante
const hCad = 1; // valor arbitrario constante
const hCin = 0; // valor arbitrario constante
...
if (medidasHz.pecho + hPecho >= medidasHz.cintura + hCin && medidasHz.pecho + hPecho >= medidasHz.cadera + hCad) {
	...
	hTotal = hCin;
	anchoMax = medidasHz.cintura + hTotal;
} else {
	hTotal = hCad;
...
export function crearMarcoDelimi() {
	...
	gMarcoDelimi.setAttributeNS(null, 'stroke', '#65c1c2');
...
export function auxConstruc() {
	...
	linSepPIzq.setAttributeNS(null, 'x1', refx + anchoMax / 2 - (medidasHz.dhp + medidasHz.seppecho) - medidasHz.dhp / 2);
	linSepPIzq.setAttributeNS(null, 'y1', refy);
	linSepPIzq.setAttributeNS(null, 'x2', refx + anchoMax / 2 - (medidasHz.dhp + medidasHz.seppecho) - medidasHz.dhp / 2);
	...
	linSepPDer.setAttributeNS(null, 'x1', refx + anchoMax / 2 - (medidasHz.dhp + medidasHz.seppecho) + medidasHz.dhp / 2);
	linSepPDer.setAttributeNS(null, 'y1', refy);
	linSepPDer.setAttributeNS(null, 'x2', refx + anchoMax / 2 - (medidasHz.dhp + medidasHz.seppecho) + medidasHz.dhp / 2);
...
export function delantero() {
	...
	// ('L', refx + anchoMax / 2 - (anchoDel + anchoCos / 2), refy) = lateral delantero hasta talle
	//REVISAR LINEA DE TALLE
	// ('T', refx - anchoMax / 2 + medidasHz.cadera / 2, refy + medidasVr.largcadera) = talle lateral delantero hasta lateral cadera delantero
...
export function espalda() {
	...
	//REVISAR LINEA DE TALLE
	// ('L', refx - anchoMax / 2 + anchoEsp + anchoCos / 2, refy) = lateral hasta talle
...
// Estan en el script body-viewer.js
// svg.appendChild(marcoDelimitador);
// svg.appendChild(axuliaresConstruccion);
